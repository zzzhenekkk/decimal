...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  ...
  if (status || (0 < fabsf(src) && fabsf(src) < 1e-28) || isnan(src) ||
    ...
    // если scale отрицательный, то мы считываем число заново, только уже через обозначением точности для флага g
    ...
    if (scale < 0) { // если была отрицательная экспа, то к ней нужно добавить длину после точки
